<template>
    <main class="shadow sm:rounded-lg" id="about">
      <div class="px-4 py-5">
        <h2 class="text-3xl my-2 py-2 text-center text-jet-black">READ</h2>
        <div class="flex flex-col items-center bg-gray-100 p-4 rounded-md">
          <p>Welcome to the Read section where you'd see emails you marked as read!</p>
        </div>
        <DashboardMenu />
        <EmailTable v-if="emails" :emails="readEmails" />
      </div>
    </main>
  </template>
  ...
  const readEmails = computed(() => emails.value?.data?.filter(email => email.isRead) || []);
